...
class AccountMove(models.Model):
    ...
    def print_gst_invoice_report(self):
        f_name = '/tmp/gst_invoice.xlsx'
        workbook = xlsxwriter.Workbook(f_name)
        worksheet = workbook.add_worksheet('GST Invoice Report')
        ...
        for obj in self:
            ...
            worksheet.merge_range('E%s:R%s' % (new_row + 1, new_row + 1), self.env.user.company_id.name,
                                  merge_format_sim)
            worksheet.merge_range('A%s:D%s' % (new_row + 2, new_row + 2), 'Address', merge_format2)
            worksheet.merge_range('E%s:R%s' % (new_row + 2, new_row + 2), self.env.user.company_id.street,
                                  merge_format_sim)
            ...
            worksheet.merge_range('J%s:R%s' % (new_row + 5, new_row + 5), 'Details of Consignee(Shipped to)',
                                  merge_format1)
            ...
            worksheet.merge_range('N%s:R%s' % (new_row + 6, new_row + 6), obj.partner_shipping_id.name,
                                  merge_format_sim)

            worksheet.merge_range('A%s:D%s' % (new_row + 7, new_row + 7), 'Address', merge_format2)
            worksheet.merge_range('E%s:I%s' % (new_row + 7, new_row + 7),
                                  str(obj.partner_id.street) + str(obj.partner_id.street2) + \
                                  str(obj.partner_id.city),
                                  merge_format_sim)
            worksheet.merge_range('J%s:M%s' % (new_row + 7, new_row + 7), 'Address', merge_format2)
            worksheet.merge_range('N%s:R%s' % (new_row + 7, new_row + 7),
                                  str(obj.partner_shipping_id.street) + str(obj.partner_shipping_id.street2) + \
                                  str(obj.partner_shipping_id.city),
                                  merge_format_sim)

            worksheet.merge_range('A%s:D%s' % (new_row + 8, new_row + 8), 'State', merge_format2)
            worksheet.merge_range('E%s:I%s' % (new_row + 8, new_row + 8), obj.partner_id.state_id.name,
                                  merge_format_sim)
            worksheet.merge_range('J%s:M%s' % (new_row + 8, new_row + 8), 'State', merge_format2)
            worksheet.merge_range('N%s:R%s' % (new_row + 8, new_row + 8), obj.partner_shipping_id.state_id.name,
                                  merge_format_sim)

            worksheet.merge_range('A%s:D%s' % (new_row + 9, new_row + 9), 'State Code', merge_format2)
            worksheet.merge_range('E%s:I%s' % (new_row + 9, new_row + 9), obj.partner_id.state_id.state_code,
                                  merge_format_sim)
            worksheet.merge_range('J%s:M%s' % (new_row + 9, new_row + 9), 'State Code', merge_format2)
            worksheet.merge_range('N%s:R%s' % (new_row + 9, new_row + 9), obj.partner_shipping_id.state_id.state_code,
                                  merge_format_sim)
            ...
            worksheet.merge_range('N%s:R%s' % (new_row + 10, new_row + 10), obj.partner_shipping_id.vat,
                                  merge_format_sim)
            ...
            for line in obj.invoice_line_ids:
                ...
                worksheet.write('D%s' % (new_row + 14), line.product_id.hsn_code.name, align_right_format1)
            ...
            worksheet.merge_range('A%s:J%s' % (new_row + 17, new_row + 17),
                                  'Certified that the Particulars given above are true and correct', merge_format)
            ...
            worksheet.merge_range('K%s:R%s' % (new_row + 19, new_row + 23),
                                  'For, ' + self.env.user.company_id.name + '\n\n\n\n\n\n\nAuthorised Signatory ',
                                  merge_format)
        ...
        data = f.read()
        f.close()
        name = 'GST Invoice Report'
        dt = 'From_' + str(fields.Date.context_today(self)) + '' + '_To_' +\
